#ifndef EARNEST_DETAIL_TXFILE_ALLOCATOR_H
#define EARNEST_DETAIL_TXFILE_ALLOCATOR_H
...
class earnest_export_ txfile_allocator final
: public abstract_tree,
  public db::db_obj
{
  private:
  ///\brief The tree is keyed on address.
  class earnest_local_ key {
    public:
  ...
  class earnest_local_ element final
  ...
  class earnest_local_ max_free_space_augment {
    public:
    static constexpr std::size_t SIZE = 8;
  ...
  txfile_allocator(std::shared_ptr<earnest::db> db, std::uint64_t off);
   ...
   */
  earnest_local_ auto maybe_allocate_txfile_bytes_from_tree_(
   ...
   * \return An address if the allocation succeeds, or an empty optional if the allocation fails.
   */
  earnest_local_ auto steal_allocate_(
      txfile::transaction& tx, std::uint64_t bytes,
  ...
  earnest_local_ void do_maintenance_(allocator_type tx_allocator);
...
#endif /* EARNEST_DETAIL_TXFILE_ALLOCATOR_H */
