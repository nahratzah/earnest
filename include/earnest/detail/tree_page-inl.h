...
inline void abstract_tree::with_equal_range_(cheap_fn_ref<void(abstract_tree_iterator, abstract_tree_iterator)> cb, const abstract_tree_page_branch_key& key) {
  ...
  if (b == nullptr) {
    ...
    while (auto next_leaf = leaf_locks.back().first->next()) {
      ...
      if (!next_leaf->elems_.empty()) {
        cb(
            abstract_tree_iterator(shared_from_this(this), next_leaf->elems_.begin().get()),
            abstract_tree_iterator(shared_from_this(this), next_leaf->elems_.begin().get()));
        return;
      }
    }
  ...
  elem_iter = std::upper_bound(
      page_iter->first->elems_.begin(), page_iter->first->elems_.end(),
      key,
      [this](const auto& x, const auto& y) -> bool {
        return less_cb(x, y);
      });
  if (elem_iter != page_iter->first->elems_.end()) e = elem_iter.get();
...
inline void abstract_tree::with_for_each_(cheap_fn_ref<stop_continue(cycle_ptr::cycle_gptr<abstract_tree_elem>)> cb) {
  ...
  do {
    for (auto iter = leaf_page->elems_.begin(), end = leaf_page->elems_.end();
        iter != end;
        ++iter) {
      const stop_continue sc = cb(iter.get());
...
inline void abstract_tree::with_for_each_augment_(
  ...
  if (auto leaf = std::dynamic_pointer_cast<tree_page_leaf>(child_page)) {
    ...
    for (auto iter = leaf->elems_.begin(); iter != leaf->elems_.end(); ++iter)
      cb(iter.get(), leaf, lck);
    return;
  }
  ...
  while (!layers.empty()) {
    ...
    if (auto leaf = std::dynamic_pointer_cast<tree_page_leaf>(child_page)) {
      LockType lck{ leaf->mtx_() };
      for (auto iter = leaf->elems_.begin(); iter != leaf->elems_.end(); ++iter) {
        const stop_continue sc = cb(iter.get(), leaf, lck);
        if (sc == stop_continue::sc_stop) return;
      }
    } else {
      layers.emplace(boost::polymorphic_pointer_downcast<tree_page_branch>(child_page), filter);
...
: cycle_ptr::cycle_base(),
  tree_page_leaf_elems_hook(static_cast<cycle_ptr::cycle_base&>(*this)),
