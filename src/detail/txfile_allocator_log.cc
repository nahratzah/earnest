...
void txfile_allocator_log::page::decode(const txfile::transaction& tx, std::uint64_t off) {
  ...
  boost::asio::read_at(tx, off, boost::asio::buffer(&rpage_, record_page::SIZE), boost::asio::transfer_all());
...
void txfile_allocator_log::page::encode(txfile::transaction& tx) const {
  ...
  copy->native_to_big_endian();
  boost::asio::write_at(tx, off, boost::asio::buffer(copy.get(), record_page::SIZE), boost::asio::transfer_all());
}
...
-> std::shared_ptr<log_entry> {
  ...
  boost::asio::write_at(tx, offset_for_idx_(idx), boost::asio::buffer(buf), boost::asio::transfer_all());
...
void txfile_allocator_log::page::write_action(
  ...
  // Perform the write.
  boost::asio::write_at(tx, offset_for_idx_(idx) + offsetof(record, act),
      boost::asio::buffer(&act_be, sizeof(act_be)), boost::asio::transfer_all());
...
void txfile_allocator_log::page::write_addr_len(
  ...
  // Perform the write.
  boost::asio::write_at(tx, offset_for_idx_(idx) + offsetof(record, addr),
      boost::asio::buffer(&addr_and_len, sizeof(addr_and_len)), boost::asio::transfer_all());
  // And update the in-memory state on commit.
  ops.on_commit(
  ...
  pages_(allocator)
{
  // Decode header.
  header h;
  boost::asio::read_at(tx, off, boost::asio::buffer(&h, header::SIZE), boost::asio::transfer_all());
...
void txfile_allocator_log::init(txfile::transaction& tx, std::uint64_t off) {
  ...
  boost::asio::write_at(tx, off, boost::asio::buffer(&h, header::SIZE), boost::asio::transfer_all());
}
...
-> maintenance_result {
  ...
  while (pages_.size() > 1u) {
    ...
    // Update offset for first page.
    {
      ...
      boost::asio::write_at(tx, off_ + offsetof(header, first_page), boost::asio::buffer(&new_first_page_be, sizeof(new_first_page_be)), boost::asio::transfer_all());
    }
...
void txfile_allocator_log::append_new_page_(
  ...
  // Write offsets.
  {
    ...
    if (pages_.empty()) {
      boost::asio::write_at(tx, off_ + offsetof(header, first_page), boost::asio::buffer(&new_page_off_be, sizeof(new_page_off_be)), boost::asio::transfer_all());
    } else {
      boost::asio::write_at(tx, pages_.back().off + offsetof(record_page, next_page), boost::asio::buffer(&new_page_off_be, sizeof(new_page_off_be)), boost::asio::transfer_all());
      ops.on_commit(
