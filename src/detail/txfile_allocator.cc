...
txfile_allocator::txfile_allocator(std::shared_ptr<earnest::db> db, std::uint64_t off)
: abstract_tree(),
  db_obj(db)
{}
...
-> std::optional<std::uint64_t> {
  ...
  boost::asio::write_at(tx, use_offset, boost::asio::buffer(buf), boost::asio::transfer_all());
...
void txfile_allocator::do_maintenance_(allocator_type tx_allocator) {
  ...
  try {
    ...
    for (auto& record : maintenance_queue) {
      ...
      if (ins_pos->succ(ins_pos_lck) != nullptr) {
        succ = boost::polymorphic_pointer_downcast<element>(ins_pos->succ(ins_pos_lck));
        ...
        while (next_page != nullptr) {
          if (!next_page->elems_.empty()) {
            succ = boost::polymorphic_pointer_downcast<element>(next_page->elems_.begin().get());
            succ_lck = std::unique_lock<std::shared_mutex>(succ->mtx_ref_());
            break;
          }
      ...
      if (succ == nullptr) {
        ...
        boost::asio::write_at(
            ...
            boost::asio::buffer(buf), boost::asio::transfer_all());
      }
