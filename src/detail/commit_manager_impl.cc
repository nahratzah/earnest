...
commit_manager_impl::commit_manager_impl(fd::offset_type off, allocator_type alloc)
...
auto commit_manager_impl::allocate(const txfile& f, fd::offset_type off, allocator_type alloc) -> std::shared_ptr<commit_manager_impl> {
  class impl
  : public commit_manager_impl
  {
    public:
    impl(fd::offset_type off, allocator_type alloc)
  ...
  boost::asio::read_at(tx, off, boost::asio::buffer(&buf, sizeof(buf)), boost::asio::transfer_all());
...
void commit_manager_impl::init(txfile::transaction& tx, fd::offset_type off) {
  commit_manager_buffer buf;
  buf.magic = magic;
  buf.tx_start = 0;
  buf.last_write = 0;
  ...
  boost::asio::write_at(tx, off, boost::asio::buffer(&buf, sizeof(buf)), boost::asio::transfer_all());
}
...
auto commit_manager_impl::do_prepare_commit_(txfile& f, allocator_type tx_alloc) -> write_id {
  ...
  if (last_write_commit_id_avail_ == 0) {
    ...
    boost::asio::write_at(immed_tx, off_ + OFF_LAST_WRITE_COMMIT_ID, boost::asio::buffer(&new_val, sizeof(new_val)), boost::asio::transfer_all());
  ...
  boost::asio::write_at(tx, off_ + OFF_COMPLETED_COMMIT_ID, boost::asio::buffer(&big_endian_commit_id, sizeof(big_endian_commit_id)), boost::asio::transfer_all());
...
void commit_manager_impl::on_completed_vacuum_(txfile& f, commit_id vacuum_target) {
  auto tx = f.begin(false);
  {
    ...
    boost::asio::write_at(tx, OFF_TX_START, boost::asio::buffer(&ntx, sizeof(ntx)), boost::asio::transfer_all());
  }
