...
struct earnest_local_ abstract_tree::page_with_lock {
...
auto abstract_tree::first_element_() -> cycle_ptr::cycle_gptr<abstract_tree_elem> {
  ...
  for (auto leaf_page = boost::polymorphic_pointer_downcast<tree_page_leaf>(pwl.page);
    ...
    if (!leaf_page->elems_.empty())
      return leaf_page->elems_.begin().get();
  }
...
auto abstract_tree::last_element_() -> cycle_ptr::cycle_gptr<abstract_tree_elem> {
  for (;;) {
    ...
    while (auto leaf_page = boost::polymorphic_pointer_downcast<tree_page_leaf>(pwl.page)) {
      ...
      if (!leaf_page->elems_.empty()) {
        result = std::prev(leaf_page->elems_.end()).get();
        break;
      }
...
restart_split:
  ...
  // Ensure we have space to perform the insert.
  if (!leaf_page->elems_.space_avail())
    goto restart_split;
  // Find the successor of the insert position.
  auto successor_elem = std::lower_bound(
      leaf_page->elems_.begin(), leaf_page->elems_.end(),
      key,
      [this](const auto& x, const auto& y) -> bool { return less_cb(x, y); });
  ...
  if (successor_elem != leaf_page->elems_.end()
    ...
    return std::make_pair(successor_elem.get(), false);
  }

  auto tx = f_->begin(false);
  tx_op_collection ops(tx_allocator);

  auto elem = elem_constructor(leaf_page->allocator, leaf_page);
  if (elem == nullptr) throw std::logic_error("tree: null insertion");
  auto ins_result = leaf_page->elems_.insert_and_shift_before(ops, successor_elem, elem);

  leaf_page->encode(tx); // XXX: maybe optimize by encoding only std::get<2>(ins_result) to std::get<3>(ins_result).
  ...
  return std::make_tuple(std::get<0>(ins_result).get(), std::get<1>(std::move(ins_result)));
}
...
: abstract_tree_page(std::move(tree), std::move(allocator)),
  elems_(*this, cfg->items_per_leaf_page, this->allocator)
{}
...
void tree_page_leaf::decode(const txfile::transaction& tx, std::uint64_t off) {
  ...
  boost::asio::read_at(tx, off, boost::asio::buffer(buf_storage), boost::asio::transfer_all());
  ...
  for (std::size_t i = 0; i < cfg->items_per_leaf_page; ++i) {
    ...
    buf += bytes_per_val;
    if (!e->is_never_visible()) {
      bool inserted [[maybe_unused]];
      std::tie(std::ignore, inserted) = elems_.insert(i, e);
      assert(inserted);
    }
  }
...
void tree_page_leaf::encode(txfile::transaction& tx) const {
  ...
  boost::asio::write_at(tx, off_, boost::asio::buffer(buf_storage), boost::asio::transfer_all());
}
...
auto tree_page_leaf::compute_augment(const std::shared_lock<std::shared_mutex>& lck, abstract_tree::allocator_type allocator) const -> std::shared_ptr<abstract_tree_page_branch_elem> {
  ...
  std::vector<cycle_ptr::cycle_gptr<const abstract_tree_elem>> elems;
  elems.reserve(elems_.size());
  for (auto iter = elems_.begin(); iter != elems_.end(); ++iter) elems.push_back(iter.get());

  return tree()->compute_augment_(off_, elems, std::move(allocator));
}
...
> {
  ...
  if (next_sibling_off_ != 0) {
    ...
    boost::asio::write_at(tx, next_sibling_off_ + offsetof(header, prev_sibling_off), boost::asio::buffer(&h.prev_sibling_off, sizeof(h.prev_sibling_off)), boost::asio::transfer_all());
  }
  ...
  boost::asio::write_at(tx, zero_begin, boost::asio::buffer(tmp), boost::asio::transfer_all());

  // Compute augments for this and sibling page.
  std::shared_ptr<abstract_tree_page_branch_elem> this_augment;
  {
    std::vector<cycle_ptr::cycle_gptr<const abstract_tree_elem>> elems;
    for (auto iter = elems_.begin(); iter != sibling_begin; ++iter) elems.push_back(iter.get());
    this_augment = t->compute_augment_(off_, elems, parent->allocator);
  }
  std::shared_ptr<abstract_tree_page_branch_elem> sibling_augment;
  {
    std::vector<cycle_ptr::cycle_gptr<const abstract_tree_elem>> elems;
    for (auto iter = sibling_begin; iter != elems_.end(); ++iter) elems.push_back(iter.get());
    sibling_augment = t->compute_augment_(new_page_off, elems, parent->allocator);
  }

  // On commit:
  // Break the pred/succ link in the elements.
  ops.on_commit(
      [elem_after_split=sibling_begin.get()]() {
        std::lock_guard<std::shared_mutex> elem_lck{ elem_after_split->mtx_ref_() };
        elem_after_split->pred_.reset();
      });
  ops.on_commit(
      [elem_before_split=std::prev(sibling_begin).get()]() {
  ...
  ops.on_commit(
      [self=shared_from_this(this), sibling_begin, new_page_off, sibling]() {
        ...
        std::for_each(
            ...
            [new_page_off, &sibling](abstract_tree_elem& elem) {
              std::lock_guard<std::shared_mutex> elem_lck{ elem.mtx_ref_() };
              elem.parent_ = sibling;
            });
...
auto tree_page_leaf::split_select_(const std::unique_lock<std::shared_mutex>& lck) -> elems_vector::iterator {
  ...
  assert(elems_.begin() < sibling_begin); // Retained range contains at least 1 element.
  assert(elems_.end() > sibling_begin); // Suffix range contains at least 1 element.
...
void tree_page_branch::insert_sibling(
  ...
  // Update size.
  {
    ...
    h.native_to_big_endian();
    boost::asio::write_at(tx, offset() + offsetof(header, size), boost::asio::buffer(&h.size, sizeof(h.size)), boost::asio::transfer_all());
  }
  ...
  boost::asio::write_at(tx, write_offset, boost::asio::buffer(buf_storage), boost::asio::transfer_all());
...
auto abstract_tree_elem::next() const -> cycle_ptr::cycle_gptr<abstract_tree_elem> {
  std::shared_lock<std::shared_mutex> self_lck{ mtx_ref_() };
  {
    auto s = succ(self_lck);
    if (s != nullptr) return boost::polymorphic_pointer_downcast<abstract_tree_elem>(s);
  }

  auto locked_page = lock_parent_for_read(self_lck);
  assert(std::get<0>(locked_page) != nullptr);
  {
    // Recheck, because parent lock may have released our lock.
    auto s = succ(self_lck);
    if (s != nullptr) return boost::polymorphic_pointer_downcast<abstract_tree_elem>(s);
  }
  ...
  for (;;) {
    ...
    auto& elems = std::get<0>(locked_page)->elems_;
    if (!elems.empty()) return elems.begin().get();
  }
}
...
auto abstract_tree_elem::prev() const -> cycle_ptr::cycle_gptr<abstract_tree_elem> {
  for (;;) {
    std::shared_lock<std::shared_mutex> self_lck{ mtx_ref_() };
    {
      auto p = pred(self_lck);
      if (p != nullptr) return boost::polymorphic_pointer_downcast<abstract_tree_elem>(p);
    }
